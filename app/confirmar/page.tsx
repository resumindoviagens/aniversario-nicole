...
export default function ConfirmarPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (allGuests.length === 0) {
      alert("Informe pelo menos um convidado.");
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      // O envio de email não deve impedir a confirmação.
    }
    ...
    <main className="min-h-screen bg-[#fff7fb] px-4 py-10">
      <section className="mx-auto max-w-5xl rounded-[40px] bg-white p-6 shadow-2xl sm:p-10">
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <button
            disabled={loading}
            className="w-full rounded-[28px] bg-gradient-to-r from-pink-600 to-pink-500 px-8 py-6 text-2xl font-black text-white shadow-2xl transition hover:scale-[1.01]"
          >
